...
import { createBedroom, createProperty } from './property-queries';
...
const schema = z.object({
  ...
  security_deposit: z.number().min(0, "Security deposit must be a positive number"),
...
export const CreatePropertyForm = ({ organizationId }: { organizationId: string }) => {
    ...
    async (data: z.infer<typeof schema>) => {
      ...
      const propertyResult = await createProperty({
        property: {
          ...
          request_pii: data.request_pin ?? false,
  ...
  const nextStep = async () => {
    ...
    const fieldsToValidate = {
      ...
      2: ['is_furnished', 'is_pets_allowed', 'is_smoking_allowed', 'is_parking_available', 'security_deposit']
    };
